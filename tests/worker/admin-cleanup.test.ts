...
async function seedDue(
  caseId: string,
  studentState: "none" | "pending" = "none",
) {
  await env.DB.batch([
    env.DB.prepare(
    ...
    env.DB.prepare(
      ...
      caseId,
      studentState,
      studentState === "pending" ? 8000 : null,
      studentState === "pending" ? 5600 : null,
    ),
    env.DB.prepare(
...
describe("verified sensitive-data cleanup", () => {
  it("deletes runtime only after all manual confirmations and ledger cleanup", async () => {
    ...
    expect(cleanupLedger).toHaveBeenCalledWith({
      caseId,
      now: "2026-08-19T00:00:00.000Z",
    });
    expect(
      await env.DB.prepare("SELECT case_id FROM cases WHERE case_id = ?")
        .bind(caseId)
        .first("case_id"),
    ).toBe(caseId);
    expect(
      await env.DB.prepare("SELECT case_id FROM case_runtime WHERE case_id = ?")
        .bind(caseId)
        .first(),
    ).toBeNull();
    expect(
      await env.DB.prepare(
        "SELECT case_id FROM submission_attempts WHERE case_id = ?",
      )
        .bind(caseId)
        .first(),
    ).toBeNull();
  ...
  it("keeps runtime data when checklist, student review or gateway is incomplete", async () => {
    ...
    await expect(
      confirmVerifiedCleanup({
        ...
        checklist,
        gateway: {
          cleanupLedger: vi.fn().mockRejectedValue(new Error("down")),
        },
        now: new Date("2026-08-19T00:00:00Z"),
      }),
