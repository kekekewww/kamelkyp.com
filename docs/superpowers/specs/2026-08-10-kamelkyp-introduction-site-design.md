...
- 不符合整理規則時，可退回請委託者修正；若雙方同意由 Kamel 整理，追加鎖定初始報價的 5%，並重新確認時程。
...
- 可使用銜接音效或效果確保順暢。
- 委託者必須提供歌曲順序與銜接時間點。
- 若需要 Kamel 協助討論與規劃順序或時間點，諮詢費為鎖定初始報價的 50%，先告知並取得同意。
- 標準時間：3–5 個工作日。
- 素材盡可能提供高音質版本。
...
「鎖定初始報價」是送出時依已填資料、匯率、學生狀態及已知加購所計算的固定基準。後續下列費用都以該基準計算，不互相複利，也不因前一次追加而提高基準：
