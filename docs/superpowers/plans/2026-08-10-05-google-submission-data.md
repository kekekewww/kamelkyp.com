...
11. Extend the generated Wrangler contract to `secrets.required = ["TURNSTILE_SECRET", "APPS_SCRIPT_URL", "APPS_SCRIPT_HMAC_SECRET"]`.
12. Extend scripts/render-worker-secrets.mjs with the same three-name allowlist. It rejects a non-HTTPS APPS_SCRIPT_URL and an HMAC secret shorter than 32 characters before writing the mode-0600 ephemeral file.
13. deploy-preview.yml reads all three values only from the Preview GitHub Environment secrets and passes the generated file to `wrangler deploy --secrets-file`; Preview points to a dedicated test Google Form and test Gmail recipient, never Production Google data.
14. tests/unit/config-contract.test.ts asserts the required-name set exactly matches the secrets-file allowlist, no value is written to .wrangler.generated.jsonc, and a missing or malformed value fails closed.
...
Expected result: no PII in D1, retry case hash enforced, duplicate Google response avoided, Preview deploy requires all three Worker secrets, and no generated secrets file is tracked or uploaded as an artifact.
